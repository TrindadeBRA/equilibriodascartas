		...
		if(check == 0){
			...
			$.ajax({
				...
				if(resp == 1){
					...
					errroTarget.html('<p style="color:green;">A mensagem foi enviada com sucesso!.</p>');
					console.log(formDetail);
				}else{
					errroTarget.html('<p style="color:red;">Ocorreu algum erro, tente novamente mais tarde.</p>');
				}
			});
		}
	});
...
jQuery(document).ready(function(){
	...
	jQuery(".celularMask").mask('(00) 00000-0000');

	// Coloca Titulo e Texto das Cartas no Modal
	jQuery(".ast_tarot_box").each(function(){
		var textoDaCarta = jQuery(this).find("p").html()
		var tituloDaCarta = jQuery(this).find("h4").text()
		jQuery(this).click(function(){
		jQuery(".modal-body").html(textoDaCarta)
		jQuery("h4#myModalLabel").text(tituloDaCarta)
		})
	})

	//Ao clicar nos icones abre o link
	jQuery('.ast_contact_info').each(function () {
		var link = jQuery(this).find("p > a").attr("href");
		jQuery(this).find("span.icone-ot").click(function(){
			window.open(link);
		});
	});
